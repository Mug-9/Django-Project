...
def get_area(area):
    area_id = 0
    try:
        area_id = PROVINCE_CODE[area]
    except Exception as e:
        area_id = CITY_CODE[area]
    return area_id
...
def get_area_name(id):
    area = '全国'
    try:
        area = list(PROVINCE_CODE.keys())[list(PROVINCE_CODE.values()).index(id)]
    except Exception as e:
        area = list(CITY_CODE.keys())[list(CITY_CODE.values()).index(id)]
    return area


def replace_citys(citys):
    news = {}
    for city in citys:
        area = get_area_name(city)
        news[area] = citys[city]
    return news


# 传入字典，
def value_index(values):
